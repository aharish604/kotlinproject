...
//        Call<EstimatedProjectsData> call = apiService.getEstimatedProjectList(sharedPreferences.getString("uid", null));
    ...
    fun getNewPostFilter() {

        val apiService = ApiClient.create()

//        Call<EstimatedProjectsData> call = apiService.getEstimatedProjectList(sharedPreferences.getString("uid", null));
        val call = apiService.getNewPostFilters()

        call?.enqueue(object : Callback<NewPostFilterData> {
            override fun onResponse(call: Call<NewPostFilterData>, response: Response<NewPostFilterData>) {
                try {
                    Log.e("cancel project ", response.body()!!.getStatus().toString())
                    ...
                    if (response.body()!!.getStatus().toString().contains("true")) {
                        ...
                        if (categoriesList.size > 0) {
//                            Log.e("filter name", categoriesList[0].itemName)
                            filterListener.newPostFilterResponseSuccess(
                                "true",
                                response.body()!!.getMessage(),
                                categoriesList
                            )
            ...
            override fun onFailure(call: Call<NewPostFilterData>, t: Throwable) {
                Log.e("TAG", t.toString())
                //                    Toast.makeText(getActivity(), t.toString(), Toast.LENGTH_SHORT).show();
            }
        })
    ...
    interface FilterListener {
        ...
        fun newPostFilterResponseSuccess(
            status: String,
            message: String?,
